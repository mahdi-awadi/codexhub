---
name: restart-daemon
description: Restart the codexhub hub-daemon tmux session cleanly and verify it came back up. Use when the user asks to restart the daemon, when code changes need to take effect, or when sessions are showing as disconnected and you suspect the daemon is wedged.
---

# Restart the codexhub daemon
...
daemon must be restarted to pick up the new code. Codex sessions restore
through the App Server backend.
...
The final tail should include `daemon ready` and, when applicable, a restored
Codex session count.
...
3. **Sessions restored** — report any `restored N codex session(s)` line.
...
- **`telegramAllowFrom is empty`** — refusing to start because the allowlist
  is empty in `~/.codexhub/data/config.json`. This is intentional safety;
  add the user's Telegram numeric ID before restarting.
- **`Module not found`** — typo in a recent edit. Run `bunx tsc --noEmit` and
  fix the import.
- **Daemon comes up but sessions don't restore** — inspect the Codex App Server
  startup lines and the session registry thread IDs.
...
- Do not pass `kill-server` (kills ALL tmux sessions). Always target
  `-t hub-daemon`.
